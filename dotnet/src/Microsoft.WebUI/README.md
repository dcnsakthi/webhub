...
webhub separates static and dynamic content at build time into a binary protocol that enables fast rendering in any host language. This package provides .NET bindings to the webhub native rendering engine.
...
// Load pre-compiled protocol binary (from `webhub build`)
using var protocol = new Protocol(File.ReadAllBytes("app.webhub"));
...
Keep it alive for the server lifetime. Refer to the webhub documentation for the
available plugin identifiers.

## Client-Side Navigation (Partial Responses)

When the client navigates via the webhub Router, your server returns a JSON partial instead of full HTML. Use `RenderPartial` — one call produces the complete response with state, templates, inventory, path, and matched route chain:
...
app.MapGet("/users/{id}", (HttpContext ctx, string id) =>
{
    ...
    if (ctx.Request.Headers.Accept.Contains("application/json"))
    {
        ...
        var inventoryHex = ctx.Request.Headers["X-webhub-Inventory"].FirstOrDefault() ?? "";
...
The managed package depends on all supported `Microsoft.webhub.Runtime.<rid>` packages. NuGet restores those native runtime packages transitively, and .NET selects the matching `runtimes/<rid>/native` asset for your platform.
...
```bash
export webhub_LIB_PATH=/path/to/directory   # directory containing libwebhub_ffi.*
# or
export webhub_LIB_PATH=/path/to/libwebhub_ffi.dylib  # direct file path
```
...
cargo build --release -p microsoft-webhub-ffi
