...
webhub started inside Microsoft Edge.
...
webhub builds directly on these platform primitives rather than wrapping them in an abstraction layer. Templates use standard HTML and native Web Components. The optional client-side router uses the Navigation API. Styling uses adopted stylesheets and CSS containment. No proprietary component model, no virtual DOM, no framework runtime in the browser.
...
webhub uses an **Islands Architecture** where each Web Component is an independent island of interactivity.
...
With webhub's Islands Architecture:
...
| webhub Islands | Only 2 components ship JS - the "Add to Cart" button and the review form. The other 8 are static HTML with zero client-side cost |
...
webhub's Rust-native handler eliminates all of this:
...
The practical result: fewer servers, lower memory consumption, more predictable latency, and lower cloud bills. A single webhub server can handle the load that previously required multiple Node.js instances behind a load balancer.

## Summary

webhub exists because modern web rendering does too much redundant work — on the server and in the browser.

| Problem | webhub's Answer |
|---------|----------------|
